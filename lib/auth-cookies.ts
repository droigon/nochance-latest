...
export class AuthCookieService {
  static setAuthCookies(user: AuthUser | Record<string, unknown>, token: string, business?: AuthBusiness | Record<string, unknown>) {
    try {
      ...
      CookieManager.setCookie('user', encodeURIComponent(JSON.stringify(userData)));
  ...
  static getStoredUser(): (AuthUser & { business?: AuthBusiness }) | null {
    try {
  ...
  static getStoredToken(): string | null {
    try {
      return CookieManager.getCookie('access_token') || localStorage.getItem('token');
    } catch {
      return null;
    }
  }
  ...
  static clearAllAuthData() {
    try {
      ...
      const authCookies = ['access_token', 'user', 'token', 'refresh_token', 'session'];
      ...
      const authLocalStorageKeys = ['token', 'user', 'business', 'access_token', 'refresh_token', 'auth_user', 'auth_token'];
